...
describe('ai profile domain', () => {
  ...
  it('OpenAI template should support responses endpoint and common model variants', () => {
    ...
    expect(openaiTemplate?.presetKey).toBe('openai')
    expect(openaiTemplate?.connectionMode).toBe('builtin')
    expect(openaiTemplate?.apiUrlBehavior).toBe('hidden')
    expect(openaiTemplate?.supportsModelDiscovery).toBe(true)
    expect(openaiTemplate?.recommendedModels).toContain('gpt-4o-mini')
    expect(openaiTemplate?.setupCopy.whatIsThis).toBeTruthy()
  ...
  it('custom template should expose advanced connection fields', () => {
    const customTemplate = AI_PROFILE_TEMPLATES.find(item => item.key === 'anthropic_compat')
    expect(customTemplate).toBeDefined()
    expect(customTemplate?.label).toBe('自定义接入')
    expect(customTemplate?.presetKey).toBe('custom')
    expect(customTemplate?.connectionMode).toBe('custom')
    expect(customTemplate?.apiUrlBehavior).toBe('required')
    expect(customTemplate?.supportsModelDiscovery).toBe(false)
  })
})
